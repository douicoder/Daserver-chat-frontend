...
(function () {
    ...
    function renderConversationList() {
        if (conversations.length === 0) {
            conversationListEl.innerHTML = '<div class="empty-conversations">No conversations yet.<br>Start a new chat!</div>';
            return;
        }
        var html = "";
        for (var i = 0; i < conversations.length; i++) {
            ...
            var unread = messages[conv.id] && messages[conv.id].__unread ? " conversation-unread" : "";
            html += '<div class="' + cls + unread + '" data-id="' + conv.id + '">';
            html += '<div class="conversation-name">' + escapeHtml(name) + '</div>';
    ...
    function renderMessages(convId, scrollToBottom) {
        ...
        for (var i = 0; i < msgs.length; i++) {
            ...
            if (showSender) {
                ...
                html += '<div class="message-header"><span class="message-time">' + formatTime(msg.created_at) + '</span></div>';
            }

            if (msg.content) {
                html += '<div class="message-content' + (isSelf ? ' self' : '') + '">' + escapeHtml(msg.content) + '</div>';
            }
    ...
    function renderAttachmentHtml(att) {
        ...
        if (isImage) {
            html += '<img src="' + backendUrl + '/api/attachments/' + att.id + '" alt="' + escapeHtml(att.original_filename) + '" loading="lazy">';
        }
        html += '<a href="' + backendUrl + '/api/attachments/' + att.id + '" target="_blank" rel="noopener">' + escapeHtml(att.original_filename) + '</a>';
        html += '<span class="attachment-meta">' + att.mime_type + ' · ' + formatFileSize(att.size) + '</span>';
        html += '</div>';
        return html;
    }
    ...
    function sendMessage(content, attachmentId) {
        ...
        var payload = { conversation_id: activeConversationId };
        if (content) payload.content = content;
        if (attachmentId) payload.attachment_id = attachmentId;

        if (window.ChatSocket && window.ChatSocket.isConnected()) {
            window.ChatSocket.sendMessage(payload.content || null, payload.attachment_id || null);
            ...
            api("/api/conversations/" + activeConversationId + "/messages", {
                ...
                if (res.status === 201 && res.data) {
                    ...
                    updateConversationListPreview(activeConversationId, res.data);
                } else {
